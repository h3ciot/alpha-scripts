...
function getGitInfo(workPath) {
    ...
    if (fs.existsSync(gitPath)) {
        ...
        let gitInfo = '';
        try {
            gitInfo = child_process.execSync(`git log -1 --date=iso  --pretty=format:"{'commit':'%H', 'date':'%ad', 'message':'%s'}"`).toString();
            gitInfo = gitInfo.replace(/\'/g,'"');
            ret = Object.assign(ret, JSON.parse(gitInfo));
            const gitHEAD = fs.readFileSync(path.resolve(gitPath,'HEAD'), 'utf-8').trim();
            // 暂不显示git registry信息
