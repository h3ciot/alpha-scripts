...
InjectGitInfoPlugin.prototype.apply = function (compiler) {
  ...
  if (compiler.hooks) {
    compiler.hooks.compilation.tap('InjectGitInfoPlugin', function (
      compilation
    ) {
      var HtmlWebpackPlugin = require('html-webpack-plugin');
      var hooks = HtmlWebpackPlugin.getHooks(compilation);
      hooks.alterAssetTags.tap('InjectGitInfoPlugin', function (
        htmlPluginData
      ) {
        ...
        const newScript = HtmlWebpackPlugin.createHtmlTagObject(
          'script',
          { type: 'text/javascript' },
          'window.gitInfo = ' + JSON.stringify(info)
        );
